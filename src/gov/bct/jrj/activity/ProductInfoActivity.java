...
public class ProductInfoActivity extends Activity {
	...
	private void showData() {
		try {
			JSONObject data = new JSONObject(this.getIntent().getStringExtra(
					"data"));
			...
			String addTime = data.getString("create_date");
			AsyncHttpClient client = new AsyncHttpClient();
			int uid = getSharedPreferences(Constants.KEY_SESSION_PREFS, 0)
					.getInt(Constants.KEY_UID, 0);
			client.get(Config.URL + "/creditLog.php?uid=" + uid + "&type=product&extra=" + URLEncoder.encode(addTime),
