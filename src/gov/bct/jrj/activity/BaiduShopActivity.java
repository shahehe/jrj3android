...
import gov.bct.jrj.activity.BaiduActivity.MyLocationListener;
...
import gov.bct.jrj.pojo.Bicycle;
...
public class BaiduShopActivity extends Activity {
	/**
	 * MapView 是地图主控件
	 */
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.baidumapshop);

		tit = (TextView) findViewById(R.id.txt_titx);

		final Bundle bundle = this.getIntent().getExtras();
		/** 社区活动名称 */
		tit.setText(bundle.getString("title"));

		BaiduNaviManager.getInstance().initEngine(BaiduShopActivity.this,
				getSdcardDir(), mNaviEngineInitListener,
				"TZSU991TNQf5IgxEFfDDLPU0", null);
		initLocation();
		...
		backBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				BaiduShopActivity.this.finish();
			}
		});

		// zuijin = (TextView) findViewById(R.id.textView1);

		mMapView = (MapView) findViewById(R.id.bmapVi);
	...
	private void loadData() {
		String url = Utils.getMetaValue(BaiduShopActivity.this, "base_url")
				+ "/rest/bike";
		// 读取缓存
		String data = Config.getCacheData(BaiduShopActivity.this, url);
		if (data == null) {
			// 读取本地
			// BicycleActvity.this.getData(url);
			// data = Config.readRawData(BicycleActvity.this,R.raw.list);
			data = Config.getCacheData(BaiduShopActivity.this, url);
	...
	public void processData(JSONObject json) {
		try {
			...
			mList = new ArrayList<Bicycle>();
			for (int i = 0; i < array.length(); i++) {
				Bicycle bicycle = new Bicycle(array.getJSONObject(i));
				mList.add(bicycle);
			}
			...
			Alerts.show(e.getMessage(), BaiduShopActivity.this);
		}
	}
	...
	public void checkUpdate(final String url, final String data) {
		if (Config.isConnected(BaiduShopActivity.this)) {
				...
				public void onSuccess(JSONArray response) {
					try {
						...
						if (md5.equals(dataMd5)) {
							Toast.makeText(BaiduShopActivity.this, "已经是最新了",
									Toast.LENGTH_SHORT).show();
						} else {
							Toast.makeText(BaiduShopActivity.this, "数据已经更新",
									Toast.LENGTH_SHORT).show();
							Config.setCacheData(BaiduShopActivity.this, url,
									json.toString());
							// processData(json);
						}
	...
	private void getData() {
		BctidAPI.get("/rest/bike", new JsonHttpResponseHandler() {
			@Override
			public void onSuccess(int statusCode, JSONArray response) {
				try {
					...
					Config.setCacheData(BaiduShopActivity.this,
							Utils.getMetaValue(BaiduShopActivity.this, "base_url")
									+ "/rest/bike", object.toString());
					// processData(object);
				} catch (Exception ex) {
					WizardAlertDialog.showErrorDialog(BaiduShopActivity.this,
			...
			public void onStart() {
				WizardAlertDialog.getInstance().showProgressDialog(
						R.string.add_data, BaiduShopActivity.this);
			}
		});
	}

	public void initOverlay(final List<Bicycle> list) {

		/** 设置中心点 */
		LatLng cenpot = new LatLng(Double.parseDouble(list.get(0).getLat()),
				Double.parseDouble(list.get(0).getLog()));
		MapStatus mMapStatus = new MapStatus.Builder().target(cenpot)
				.zoom(15.0f).build();
		...
		for (int i = 0; i < list.size(); i++) {
			LatLng latLng = new LatLng(
					...
					Double.parseDouble(list.get(i).getLog()));
		...
		mBaiduMap.setOnMarkerClickListener(new OnMarkerClickListener() {
			...
			public boolean onMarkerClick(Marker arg0) {
				...
				View infoWindowView = LayoutInflater.from(BaiduShopActivity.this)
						.inflate(R.layout.infowindow_view, null);
				TextView message = (TextView) infoWindowView
						.findViewById(R.id.text1);

				message.setText(b.getContent());
		...
		BaiduNaviManager.getInstance().launchNavigator(this, startPoint, // 起点（可指定坐标系）
				...
				new OnStartNavigationListener() { // 跳转监听
					...
					public void onJumpToNavigator(Bundle configParams) {
						Intent intent = new Intent(BaiduShopActivity.this,
	...
	public void initLocation() {
		mLocClient = new LocationClient(BaiduShopActivity.this);
	 ...
	 * 计算最近距离的5个点
	 */
	private void getNearbyPoint(List<Bicycle> mlist) {

		List<Bicycle> tempList = new ArrayList<Bicycle>();
		Collections.addAll(tempList, new Bicycle[mlist.size()]);
		...
		for (int i = 0; i < size - 1; i++) {
			...
			for (int j = 0; j < size - i - 1; j++) {
				...
				if (distant1 > distant2) {
					Bicycle tempBicycle = tempList.get(j);
		...
		if (tempList.size() > 5) {
			...
			List<Bicycle> mlist2 = new ArrayList<Bicycle>();
			for (int i = 0; i < 5; i++) {
